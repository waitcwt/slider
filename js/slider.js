...
function initSlider(option){
        ...
        slider.prototype.control= function(flag,jump){
            ...
            this.trigger('berforeslide',[cur,conf.index]);
            if(conf.shape==='slide'){
        ...
        slider.prototype.scroll = function(current){
            ...
            conf.Handle.animate(this.animateObj(direction,slide),400,'swing',function(){that.endBack();});
        ...
        slider.prototype.fade =function(current){
            ...
            domselector.eq(conf.index).css('z-index',-1).fadeOut('normal',function(){});
            domselector.eq(conf.current).css('z-index',1).fadeIn('normal',function(){that.endBack();});
        };
        ...
        slider.prototype.endBack= function(){
            conf.canClick=true;
            this.trigger('afterslide',[conf.current,conf.index]);
